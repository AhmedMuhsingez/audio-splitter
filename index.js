...
const run = async () => {
	...
	inFiles.forEach(async (inFile) => {
		const inFilename = inFile.split(".")[0];

		const outFolder = await deleteOldAndCreateNewFolder(`./out${inFilename}`);

		const paddedOutFolder = await deleteOldAndCreateNewFolder(`./out-pad${inFilename}`);
...
const generateRandomPadding = (rng) => {
	const randomValue = Math.round((rng() * 0.6 + 0.7) * 1000) / 1000;
	return randomValue;
};
